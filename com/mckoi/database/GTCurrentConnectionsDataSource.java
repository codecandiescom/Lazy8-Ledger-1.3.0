...
final class GTCurrentConnectionsDataSource extends GTDataSource {
  ...
  public DataCell getCellContents(final int column, final int row) {
    switch (column) {
      case 0:  // username
        return new StringDataCell(1500,
                          (String) key_value_pairs.get(row * 4));
      case 1:  // host_string
        return new StringDataCell(1500,
                          (String) key_value_pairs.get((row * 4) + 1));
      case 2:  // last_command
        return new TimeDataCell((Date) key_value_pairs.get((row * 4) + 2));
      case 3:  // time_connected
        return new TimeDataCell((Date) key_value_pairs.get((row * 4) + 3));
      default:
  ...
  static {
    ...
    def.setName("sUSRCurrentConnections");
