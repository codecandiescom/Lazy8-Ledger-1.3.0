...
public final class DataTable extends DefaultDataTable {
  ...
  DataTable(DatabaseConnection connection,
    ...
    super(connection.getDatabase(),
          data_source.getDataTableDef().getName(),
          data_source.getDataTableDef().toTableFieldArray());
...
//    System.out.println("NEW DATATABLE");
  ...
  void addRowToColumnSchemes(int row_number) {
    // This is handled by DataTableManager now.
   ...
   */
  public ReferenceTable declareAs(String new_name) {
  ...
  /**
   * Creates an empty 'null' DataCell object for the given column.
   */
  DataCell createDataCellObject(int column) {
    checkSafeOperation();  // safe op

    return getFieldAt(column).createDataCell();
  }
...
//   * list.
//   */
//  private String stringColumnList(String[] list) {
...
//  /**
//   * Checks the uniqueness of the columns in the given RowData object.  If
//   * the given column information in the row data is not unique then it
//   * returns false.
//   */
//  private boolean isUniqueColumns(RowData row_data, String[] cols) {
...
//      col_indexes[i] =
//                   fastFindFieldName(new Variable(getTableName(), cols[i]));
//    }
//
...
//      DataCell cell = row_data.getCellData(col_index);
//
//      String column_name = column_def.getName();
//
...
//
//        SelectableScheme ss = getSelectableSchemeFor(
//                                             col_index, col_index, this);
...
////         "You tried to add a duplicate row to table '" + getName() +
////         "'.  There were cells in the row that clashed with the unique " +
////         "columns.");
//    }
//    return true;
//
...
//      DataCell cell = row_data.getCellData(i);
//
//      // Check: Column defined as not null and cell being inserted is not null.
...
//            getResolvedColumnName(i) + "' which is declared as 'not_null'");
//      }
//
...
//    SchemaManager.UniqueSet unique_constraint =
//                   schema_manager.queryTablePrimaryKey(
//                               table_def.getSchema(), table_def.getName());
//    if (unique_constraint != null &&
//        unique_constraint.deferred == SchemaManager.INITIALLY_IMMEDIATE) {
//      if (!isUniqueColumns(row_data, unique_constraint.columns)) {
//        throw new DatabaseConstraintViolationException(
//          DatabaseConstraintViolationException.PRIMARY_KEY_VIOLATION,
//          "Immediate primary Key constraint violation (" +
//          unique_constraint.name + ") Columns = ( " +
//          stringColumnList(unique_constraint.columns) +
//          " ) Table = ( " + table_name.toString() + " )");
//      }
//    }
//
//    // Check any unique constraints.
//    SchemaManager.UniqueSet[] unique_constraints =
//                   schema_manager.queryTableUniqueGroups(
//                               table_def.getSchema(), table_def.getName());
//    for (int i = 0; i < unique_constraints.length; ++i) {
//      SchemaManager.UniqueSet unique = unique_constraints[i];
//      if (unique.deferred == SchemaManager.INITIALLY_IMMEDIATE) {
//        if (!isUniqueColumns(row_data, unique.columns)) {
//          throw new DatabaseConstraintViolationException(
//            DatabaseConstraintViolationException.UNIQUE_VIOLATION,
//            "Immediate unique constraint violation (" + unique.name +
//            ") Columns = ( " + stringColumnList(unique.columns) +
//            " ) Table = ( " + table_name.toString() + " )");
//        }
//      }
//    }
...
//    SchemaManager.CheckSet[] check_constraints =
//                     schema_manager.queryTableCheck(
//                              table_def.getSchema(), table_def.getName());
//    for (int i = 0; i < check_constraints.length; ++i) {
//      SchemaManager.CheckSet check = check_constraints[i];
//      if (check.deferred == SchemaManager.INITIALLY_IMMEDIATE) {
//        Expression exp = check.expression;
//
//        Object ob = row_data.evaluate(exp);
...
//               "Immediate check constraint violation (" + check.name +
//               ") - '" + exp.text() +
...
//            "Immediate check constraint violation (" + check.name + ") - '" +
//            exp.text() + "' returned a non boolean result.");
//        }
//
//      }
//    }
...
//      Debug().writeException(Lvl.ALERT, ex);
//      // If there was a problem adding the new modification, add the original
//      // back in.
////      addRow(original_data);
...
//    }
  ...
  // For javascript, 'delete' is a reserved word.
  public int _delete(Table table) throws DatabaseException {
    return delete(table);
  }

  /**
   * Updates the table by applying the assignment operations over each row
   * that's found in the input 'table' set.  The input table must be a direct
   ...
   * <p>
   * NOTE: We assume there's no duplicate rows to the root set from the
   *   given 'table'.
   */
...
//      }
  ...
  public int compareCellTo(DataCell ob, int column, int row) {
    ...
    return data_source.compareCellTo(ob, column, row);
  }
    ...
    if (LOCK_DEBUG) {
      if (lock_type == Lock.READ) {
        ...
        if (debug_write_lock_count > 1) {
          throw new Error(">1 write lock on table " + getName());
        }
      }
      else {
...
//    if (thread instanceof UserWorkerThread) {
//      UserWorkerThread work_thread = (UserWorkerThread) thread;
//      DatabaseConnection db_connection = work_thread.getDatabaseConnection();
//      if (db_connection != null) {
//        LockingMechanism locker = db_connection.getLockingMechanism();
//        return locker.isInExclusiveMode();
//      }
//      // Assume if no DatabaseConnection, then safe.
//    }
//    // Assume we are in exclusive mode if thread not a user worker thread...
//    // ( bad assumption? )
//    return true;
  }
...
//      boolean is_internal_table = getName().startsWith("sUSR");
////                                  && isInExclusiveMode();
      ...
      if (!(is_internal_table ||
        ...
        Debug().writeException(new RuntimeException(
                     "Invalid read access on table '" + getName() + "'"));
      }
    }
  }
  ...
  private void checkReadWriteLock() {
    if (LOCK_DEBUG) {
      ...
      if (!(debug_write_lock_count == 1 || isInExclusiveMode())) {
        Debug().writeException(
               new RuntimeException("Invalid read/write access on table '" +
                                    getName() + "'"));
      }
    }
  }
  ...
  public TableField getFieldAt(int column) {
    ...
    return super.getFieldAt(column);
  }

  /**
   * @deprecated use getResolvedVariable instead
   */
  public String getResolvedColumnName(int column) {
    checkSafeOperation();  // safe op

    return super.getResolvedColumnName(column);
  }
  ...
  public int findFieldName(String name) {
    ...
    return super.findFieldName(name);
  }
