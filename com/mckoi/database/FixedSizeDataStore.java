...
//  private static int unique_store_key = 0;
//  private int this_store_key;
...
//  /**
//   * If this is set to false, then this store prevents writing records that
//   * traverse multiple sectors.  This is an assertion that can be enforced
//   * for certain types of stores.
//   */
//  private boolean allow_write_across = true;
  ...
  public FixedSizeDataStore(File data_file, int sector_size,
    ...
    this.debug = logger;
//    ++unique_store_key;
//    this_store_key = unique_store_key;
...
//  /**
//   * This sets the assertion that no write across operations are permitted.
//   */
//  public void setAllowWriteAcross(boolean status) {
//    allow_write_across = status;
//  }
    ...
    if (buffered_sector != sector) {
      ...
      if (sector_cache != null) {
        ...
        if (sbuf == null) {
          ...
          seekSector(sector);
          data_store.read(sector_buffer, 0, sector_size);
...
//  // A buffer for setting up the extended deleted chain.
//  byte[] delete_info = new byte[5];
//  /**
//   * Extends the size of the data store by to at least the given size.  If
//   * this grows the file size by more than one sector then the additional
//   * sectors are added to the delete chain.  Note that the first sector of
//   * the new set is not put into the deleted chain.
//   */
//  private void extendDataStoreByAtLeast(int sector, long size)
//                                                       throws IOException {
//    // [ We can't use this because of the stream writing methods that
//    //   allocate a block of sectors at a time.  Also, if the file is
//    //   locked we are going to eat up disk space like nothing else (File
//    //   will grow 16 times as fast). ]
//
//    int old_delete_head = delete_head;
//    // The number of sectors to grow the file size by.
//    int grow_by_amount = 16;
//
//    // Additional sectors to allocate,
//    setDataStoreSize(size + (grow_by_amount * sector_size));
//
//    // Set the new head of the delete chain
//    delete_head = sector + 1;
//    delete_info[0] = DELETED;
//    // For each additional sector, set the sector as deleted and link with the
//    // chain.
//    for (int i = 0; i < grow_by_amount - 1; ++i) {
//      ByteArrayUtil.setInt(delete_head + i + 1, delete_info, 1);
//      seekSector(delete_head + i);
//      data_store.write(delete_info, 0, 5);
//    }
//    // Set the last sector of the new set to the old delete_head
//    ByteArrayUtil.setInt(old_delete_head, delete_info, 1);
//    seekSector(delete_head + grow_by_amount - 1);
//    data_store.write(delete_info, 0, 5);
//    // Synchronize
//    synch();
//  }
...
//      extendDataStoreByAtLeast(sector, seek_to + sector_size);
  ...
  public void readReservedBuffer(byte[] info, int offset, int length)
    ...
    data_store.seek(64);
    data_store.read(info, offset, length);
  }
...
//      // NOTE: For better efficiency, this should be buffered to a byte[]
//      //   array then written out.
//      data_store.writeLong(delete_head);
//      data_store.writeLong(used_sector_count);
    }
  }
...
//      // Fill with '255'
//      for (int i = 0; i < SECTOR_DATA_OFFSET; ++i) {
//        bout.write(255);
//      }
//      data_store.seek(0);
      // Write the magic int
...
//        System.out.println("Pred Size: " + pred_size);
        ...
        if (need_repair) {
          debug.write(Lvl.ALERT, this,
                      "Need to repair store - not closed cleanly.");
//          repair();
        }
...
//  /**
//   * Traverses a sector chain counting the number of chains until the
//   * terminating sector is reached.  Useful for diagnostic and statistical
//   * operations.
//   */
//  public int chainCount(int sector_head) throws IOException {
//
//    if (sector_head >= sectorCount()) {
//      throw new IOException("Can't get sector, out of range.");
//    }
...
//    System.out.println(FixedSizeDataStore.this + " DELETE ACROSS: " + sector_head);
...
//    if (!allow_write_across) {
//      throw new IOException("Store does not allow 'writeAcross'");
//    }
...
//    // Go through the delete chain and make sure we can count all the way
//    // through it.
//    int walk = delete_head;
//    int delete_count = 0;
//    while (walk != -1) {
//      ++delete_count;
//      readSector(walk);
//      // Are we marked as deleted?
//      if ((sector_buffer[0] & DELETED) == 0) {  // Not deleted
//        terminal.println(
//           "ERROR: Sector " + walk +
//           " not marked as deleted but in deleted chain.");
//      }
//
//      walk = ByteArrayUtil.getInt(sector_buffer, 1);
//    }
//
//    // The number of sectors used (from the variable).
//    int sectors_used = getSectorUseCount();
//
//    // If the number of sectors used is greater than the sector count then
//    // the file has probably been truncated.
//    if (sectors_used > sector_count) {
//      terminal.println(
//               "! Sectors used > sector count.  Probable file truncation.");
//    }
//    else {
//      terminal.println("- Sectors Used: " + getSectorUseCount());
//
//      // Does the number deleted match what we predict?
//      int predicted_del = sectorCount() - getSectorUseCount();
//      if (delete_count != predicted_del) {
//        terminal.println(
//           "! ERROR: Counted " + delete_count + " entries in the delete " +
//           "chain however, sectorCount() - getSectorsUsed() reports " +
//           predicted_del);
//      }
//
//    }
...
//    if (allow_write_across) {
//      // Not allowed to repair a store that allows 'writeAcross'.  This is
//      // because repair reorganizes the deleted chain count which may be used
//      // when reading deleted sectors.
//      throw new IOException(
//                         "Can not repair a store that allows 'writeAcross'");
//    }
...
//  /**
//   * Checks that the file is correct.  This is an expensive operation and
...
//
//    System.out.println("Sector Count: " + sectorCount());
//    System.out.println("Sectors Used: " + getSectorUseCount());
//
//    // Does the number deleted match what we predict?
//    int predicted_del = sectorCount() - getSectorUseCount();
//    if (delete_count != predicted_del) {
//      System.out.println(
//         "ERROR: Counted " + delete_count + " entries in the delete chain " +
//         "however, sectorCount() - getSectorsUsed() reports " + predicted_del);
//    }
//
//  }
...
//      System.out.println(FixedSizeDataStore.this + "IS LOCKED: " + locked());
//      System.out.println(FixedSizeDataStore.this + "DELETE HEAD: " + delete_head);
...
//        if (last_sector != -1) {
//          if (sectors[0] != last_sector) {
//            System.out.println(sectors[0]);
//            System.out.println(last_sector);
//            throw new Error("Sectors not chained correctly.");
//          }
//        }
...
//        }
//        System.out.println("1 WRITING: " + cur_sector);
//        System.out.println("1 CHAINING TO: " + next_sector);
...
//        // Then flush to the next sector.
//        int[] sectors = findFreeSectors(2);
//        if (last_sector != -1) {
//          if (sectors[0] != last_sector) {
//            throw new Error("Sectors not chained correctly.");
//          }
...
//          first_sector = sectors[0];
//        }
//        writeBufToSector(sectors[0], sectors[1], buf, 0, index);
...
//        System.out.println("2 WRITING: " + cur_sector);
//        System.out.println("2 CHAINING TO: " + next_sector);
        writeBufToSector(cur_sector, next_sector, buf, 0, index);
        cur_sector = next_sector;
...
//      System.out.println(FixedSizeDataStore.this + "3 WRITING: " + cur_sector);
//      System.out.println(FixedSizeDataStore.this + "3 CHAINING TO: -1");
    }
