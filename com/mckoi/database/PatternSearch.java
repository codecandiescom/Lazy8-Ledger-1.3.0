...
public final class PatternSearch {
   ...
   */
  public static boolean fullPatternMatch(String pattern, String str,
                                         char escape_char) {
...
//      }
  ...
  static IntegerVector search(Table table, int column, String pattern,
    ...
    // If the column type is not DB_STRING then report an error.

    TableField col_field = table.getFieldAt(column);
    if (col_field.getType() != Types.DB_STRING) {
      throw new RuntimeException("Unable to perform a pattern search on a non-String type table column.");
    }
...
//    // A temporary object for creating ValueSubstitution objects.
...
//      val = new ValueSubstitution(Types.DB_STRING, pattern);
//      DataCell cell = DataCellFactory.createDataCell(col_field, val);
      DataCell cell = DataCellFactory.fromObject(col_field.getType(), pattern);
      return table.selectRows(column, Condition.EQUALS, cell);

      // RETURN
    }
    else if (pre_pattern.length() == 0) {
      // No pre-pattern easy search :-(.  Just select all from the column in
      // this case.
      ...
      DataCell cell_lower = DataCellFactory.fromObject(
                                            Types.DB_STRING, lower_bounds);
      DataCell cell_upper = DataCellFactory.fromObject(
                                            Types.DB_STRING, upper_bounds);
//      val = new ValueSubstitution(Types.DB_STRING, lower_bounds);
//      DataCell cell_lower = DataCellFactory.createDataCell(col_field, val);
//      val = new ValueSubstitution(Types.DB_STRING, upper_bounds);
//      DataCell cell_upper = DataCellFactory.createDataCell(col_field, val);
...
//    }
    ...
    while (iterator.hasNext()) {
      ...
      DataCell cell = table.getCellContents(column, iterator.next());
      // Null values doesn't match with anything
      if (!cell.isNull()) {
        String expression = (String) cell.getCell();
