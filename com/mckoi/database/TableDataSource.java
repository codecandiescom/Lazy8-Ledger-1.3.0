...
public interface TableDataSource {
   ...
   * in the table.  This will generally be an expensive operation, so calls
  ...
  /**
   * Compares the cell to the given cell in the table.  The cell may only
   * be one of the types allowed in the database. This will generally be an
   * expensive operation, so calls to it should be kept to a minimum.  Note
   * that the offset between two rows is not necessarily 1.  Use
   * 'rowEnumeration' to create a row iterator.
   *
   * @returns LESS_THAN, GREATER_THAN, or EQUAL
   */
  int compareCellTo(DataCell ob, int column, int row);
