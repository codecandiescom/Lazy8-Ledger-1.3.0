...
//    rid_list = new IntegerVector();
   ...
   */
  private DataCell getCellContents(int row) {
...
//    System.out.println(hash_rid_difference);
   ...
   */
  void insertRID(DataCell cell, int row) {
        ...
        synchronized (modification_lock) {
          ...
          for (int r = 0; r < set_size; ++r) {
            if (!master_table.recordDeleted(r)) {
              DataCell cell = getCellContents(r);
        ...
        synchronized (master_table) {
          synchronized (modification_lock) {
            ...
            while (i < mod_size) {
              ...
              if (type == 1) {
                DataCell cell =
                            (DataCell) concurrent_modification_data.get(m_data);
                insertRID(cell, row);
                ++m_data;
                ++insert_count;
              }
