...
public class AlterTable extends Statement {
  ...
  public void prepare() throws DatabaseException {
    ...
    if (create_statement != null) {
      ...
      create_stmt.init(database, create_statement);
      create_stmt.prepare();
...
//    tname = TableName.resolve(db.getCurrentSchema(), table_name);
  ...
  public Table evaluate() throws DatabaseException {
    ...
    if (!user.canAlterTable(database, tname)) {
    ...
    if (create_statement != null) {
      ...
      for (int i = 0; i < actions.size(); ++i) {
        ...
        if (action.getAction().equals("DROP_CONSTRAINT")) {
          database.dropNamedConstraint(tname, (String) action.getElement(0));
        }
  ...
  public boolean isExclusive() {
    // Yes, alter table operations are exclusive.
    return true;
  }

  public List readsFromTables() {
    // Exclusive operation so no tables...
