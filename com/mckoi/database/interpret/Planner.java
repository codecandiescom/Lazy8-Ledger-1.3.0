...
public class Planner {
  ...
  private static void prepareSearchExpression(
    ...
    expression.prepare(new ExpressionPreparer() {
      ...
      public Object prepare(Object element) throws DatabaseException {
        ...
        return new QueryPlan.CachePointNode(sq_plan);
      }
    });
  ...
  private static Expression filterHavingClause(Expression having_expr,
    ...
    if (having_expr.size() > 1) {
      ...
      Expression new_left = filterHavingClause(exps[0], aggregate_list);
      Expression new_right = filterHavingClause(exps[1], aggregate_list);
      Expression expr = new Expression(new_left, op, new_right);
      return expr;
    }
    else {
      // Not logical so determine if the expression is an aggregate or not
      if (having_expr.hasAggregateFunction()) {
...
//        sub_query_from_set.setParent(from_set);
      ...
      // Else must be a standard query table,
        ...
        // a regular table.
        DataTable data_table = db.getTable(table_name);
//        // Mark this table as 'selected from' within the transaction.
//        db.addSelectedFromTable(table_name);
        ...
        if (alias == null) {
          ...
          // Aliased table - Get the table declared under a different name.
          given_name = new TableName(alias);
          table = data_table.declareAs(alias);
        }
        FromTableDirectSource source = new FromTableDirectSource(db, table,
                                         unique_key, given_name, table_name);
        from_set.addTable(source);
      }
    }  // while (tables.hasNext())
  ...
  public static QueryPlanNode formQueryPlan(DatabaseConnection db,
    ...
    for (int i = 0; i < from_set.setCount(); ++i) {
      ...
      if (table instanceof FromTableSubQuerySource) {
        ...
        QueryPlanNode ds_plan =
                new QueryPlan.FetchTableNode(root_name, aliased_name);
    ...
    for (int i = 0; i < join_set.getTableCount() - 1; ++i) {
      ...
      if (type == JoiningSet.INNER_JOIN) {
        // If this is an inner join and we have an ON expression, then simply
        // append the expression to the end of the 'where' search expression.
        if (on_expression != null) {
          where_clause.appendExpression(on_expression);
        }
      }
      else {
        // Otherwise, resolve the on_expression
    ...
    for (int i = 0; i < gsz; ++i) {
      ...
      if (group_by_expression != null) {
        if (group_by_expression.hasAggregateFunction()) {
...
//      List all_vars = having_clause.getFromExpression().allVariables();
...
//        System.out.println(exp);
  ...
  private static class QuerySelectColumnSet {
    ...
    Variable addHiddenFunction(String fun_alias, Expression function) {
      ...
      if (function.hasAggregateFunction()) {
        ++aggregate_count;
      }
     ...
     */
    private void prepareSelectColumn(SelectColumn col)
                                                  throws DatabaseException {
      ...
      if (v == null) {
        ...
        // If this is an aggregate column then add to aggregate count.
        if (col.expression.hasAggregateFunction()) {
          ++aggregate_count;
    ...
    void planForExpressionList(List and_list) {
      ...
      for (int i = 0; i < and_list.size(); ++i) {
        ...
        if (!(lob instanceof Operator) ||
          ...
          andexp.addElement(Boolean.TRUE);
...
//        else {
//          throw new RuntimeException("Unhandled expression: " + andexp);
//        }
      }
...
//          Expression[] exps = exp.split();
//          list = createAndList(list, exps[0]);
//          list = createAndList(list, exps[1]);
//          return list;
//        }
//      }
