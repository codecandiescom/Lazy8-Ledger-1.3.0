...
//      for (int n = 0; n < INITIAL_BLOCK_SPARE_SPACE; ++n) {
//        index_table_list.putInt(0);
//        index_table_list.putShort((short) 0);
//      }
    }
  }
...
//  private boolean concurrency_test = false;
...
//      // Check sorted
//      long min = -1;
//      for (int i = 0; i < memory_index_set_list.size(); ++i) {
//        SnapshotIndexSet set = (SnapshotIndexSet) memory_index_set_list.get(i);
//        long sid = set.getID();
//        if (sid > min) {
//          min = sid;
//        }
//        else {
//          throw new Error("memory_index_set not sorted");
//        }
//      }
//      min = -1;
//      for (int i = 0; i < index_set_garbage.size(); ++i) {
//        SnapshotIndexSet set = (SnapshotIndexSet) index_set_garbage.get(i);
//        long sid = set.getID();
//        if (sid > min) {
//          min = sid;
//        }
//        else {
//          throw new Error("index_set_garbage not sorted");
//        }
//      }
...
//        System.out.println(index_set_garbage.size());
        if (deleted) {
...
//                            sz + " sectors from IndexStore '" + file + "'");
//            }
          }
          catch (IOException e) {
...
//    try {
//      System.out.println("Index status: " + statusString());
//    }
//    catch (IOException e) {
//      e.printStackTrace();
//    }
...
//     * For debugging.
//     */
//    private Error DEBUG_CONSTRUCTOR;
...
//      // Copy the snapshot of the buffer in to a new ByteBuffer object.
//      byte[] new_buf = new byte[in_buf.limit()];
//      in_buf.position(0);
//      in_buf.get(new_buf, 0, new_buf.length);
//      this.buf = new ByteBuffer(new_buf);
//      this.buf_length = length;
...
//        sector_list = new int[list_size];
//        block_size = new short[list_size];
...
//          sector_list[i] = buf.getInt();
//          block_size[i] = buf.getShort();
        }
...
//        byte[] arr = new byte[block_count * 4];
        ...
        synchronized (IndexStore.this) {
          try {
//            arr = index_store.getSector(index_sector, arr, 0, arr.length);
...
//        for (int i = 0; i < count; ++i) {
//
//          c1 = (((int) arr[p + 0]) & 0x0FF);
//          c2 = (((int) arr[p + 1]) & 0x0FF);
//          c3 = (((int) arr[p + 2]) & 0x0FF);
//          c4 = (((int) arr[p + 3]) & 0x0FF);
