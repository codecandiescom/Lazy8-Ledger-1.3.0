...
//      System.out.println("right_col_map: " + right_col_map[i]);
      ...
      TableField left_field = left_table.getFieldAt(left_col_map[i]);
      TableField right_field = right_table.getFieldAt(i);
      if (left_field.getType() != right_field.getType()) {
        throw new RuntimeException(
            "The type of the sub-query expression " + left_vars[i] + "(" +
            left_field.getSQLTypeName() + ") is incompatible with " +
            "the sub-query type " + right_field.getSQLTypeName() + ".");
      }
    }
    ...
    if (op.isSubQueryForm(Operator.ALL)) {
      ...
      if (op.is(">") || op.is(">=")) {
        ...
        DataCell[] cells = right_table.getSingleCellContent(right_col_map);
        if (cells != null) {
