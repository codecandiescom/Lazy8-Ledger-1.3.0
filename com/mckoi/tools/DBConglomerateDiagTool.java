...
public class DBConglomerateDiagTool {
  ...
  private static void diagnose(String path, String name) {
    try {
      ...
      // Open it.
      conglomerate.open(new File(path), name, true); //false);
  ...
  public static void main(String[] args) {
    ...
    Debug.setDebugLevel(Debug.INFORMATION);
...
//    frame.setVisible(true);
  ...
  private class DTableModel extends AbstractTableModel {
    ...
    public int getColumnCount() {
      return diag_table.getDataTableDef().columnCount() + 3;
    }
    ...
    public String getColumnName(int col) {
      if (col == 0) {
        ...
        return diag_table.getDataTableDef().columnAt(col - 3).getName();
      }
    }
    ...
    public Object getValueAt(int row, int col) {
      try {
        ...
        if (col == 0) {
          ...
          return new Integer(diag_table.recordSize(row));
        }
        else {
          return diag_table.getCellContents(col - 3, row).getCell();
        }
      }
      catch (Throwable e) {
