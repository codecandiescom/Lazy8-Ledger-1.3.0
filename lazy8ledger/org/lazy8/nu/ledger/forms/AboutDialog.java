...
public class AboutDialog extends EnhancedDialog
{
	public AboutDialog(View view)
	{
		...
		close = new JButton(jEdit.getProperty("lazy8ledger.common.close"));
...
//			setForeground(new Color(206,206,229));
			...
			StringTokenizer st = new StringTokenizer(
				jEdit.getProperty("lazy8ledger.about.text"),"\n");
		...
		public void paintComponent(Graphics _g)
		{
			...
			for(int i = firstLine; i <= lastLine; i++)
			{
				if(i >= 0 && i < text.size())
				{
					...
					g.drawString(line, (340
						- fm.stringWidth(line)) / 2,y);
				}
				y += fm.getHeight();
			}

			String version = "Lazy 8 Ledger " + jEdit.getProperty("plugin.lazy8ledger.Lazy8LedgerPlugin.version");
			g.drawString(version,(340 - fm.stringWidth(version)) / 2,
				370);
		}
